...
RSpec.feature 'Contact user', type: :feature do
  ...
  context 'logged in user' do
    ...
    scenario "successfully sends a message to a post's author", :js do
      visit post_path(post)

      expect(page).to have_selector('.contact-user form')

      fill_in('message_body', with: FFaker::Lorem.paragraph)
    ...
    scenario 'sees an already contacted message', :js do
      create(
        :private_conversation_with_messages,
